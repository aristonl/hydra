...
void _kprintf(PrintData* pd, const char* prefix, const char* prefix2, const char* str) {
    ...
    if (spaces > len)
        ...
        spaces = 0;
    if (spaces > 0 && pd->right == 0 && pd->fill != '0')
        _fill(pd, spaces);
...
int kprintf(const char* fmt, ...) {
    ...
    va_start(args, fmt);
    for (int i = 0; fmt[i]; i++) {
        if (!pd.is_format && fmt[i] != '%') {
            kputchar(fmt[i]);
        } else if (!pd.is_format)
            pd.is_format = true;
            ...
            // Get format
            if (fmt[i] == 'p')
                pd.num = (unsigned long long)va_arg(args, void*);
            else if (fmt[i] == 'd') {
            ...
            *x-- = 0;
            ...
            x++;
            if (pd.baseprefix && pd.base == 16)
                bprefix = "0x";
            else if (pd.baseprefix && pd.base == 8)
                bprefix = "0";
            else
                bprefix = "";
