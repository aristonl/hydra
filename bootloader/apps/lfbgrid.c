...
static void fill_boxes(UINT32 *PixelBuffer, UINT32 Width, UINT32 Height, UINT32 Pitch, EFI_GRAPHICS_PIXEL_FORMAT Format, EFI_PIXEL_BITMASK Info) {
	UINT32 Red, Green, y, x, color;
	switch(Format) {
	...
	for (y = 0; y < Height; y++)
		...
		for (x = 0; x < Width; x++)
			if (x % 32 == 0 && x != 0) color = (color == Red) ? Green : Red;
			PixelBuffer[y * Pitch + x] = color;
}

static void draw_boxes(EFI_GRAPHICS_OUTPUT_PROTOCOL *gop) {
	int i, imax;
	EFI_STATUS rc;
	...
	#endif
	if (gop->Mode) imax = gop->Mode->MaxMode;
	else {
	...
	for (i = 0; i < imax; i++) {
		...
		if (CompareMem(info, gop->Mode->Info, sizeof (*info))) continue;
		NumPixels = info->VerticalResolution * info->PixelsPerScanLine;
		BufferSize = NumPixels * sizeof(UINT32);
		if (BufferSize == gop->Mode->FrameBufferSize) CopySize = BufferSize;
		else {
			CopySize = BufferSize < gop->Mode->FrameBufferSize ? BufferSize : gop->Mode->FrameBufferSize;
		...
		fill_boxes(PixelBuffer, info->HorizontalResolution, info->VerticalResolution, info->PixelsPerScanLine, info->PixelFormat, info->PixelInformation);
...
static EFI_STATUS SetWatchdog(UINTN seconds) {
	EFI_STATUS rc;
