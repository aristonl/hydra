...
static int is_device(EFI_PCI_IO *pciio, uint16_t vendor_id, uint16_t device_id) {
	...
	if (EFI_ERROR(rc)) return 0;
	if (vendor_id == lpcif.vendor_id && device_id == lpcif.device_id) return 1;
	return 0;
}

static EFI_STATUS find_pci_device(uint16_t vendor_id, uint16_t device_id, EFI_PCI_IO **pciio) {
	EFI_STATUS rc;
	EFI_HANDLE *Handles;
	UINTN NoHandles, i;
	if (!pciio) return EFI_INVALID_PARAMETER;
	rc = LibLocateHandle(ByProtocol, &PciIoProtocol, NULL, &NoHandles, &Handles);
	if (EFI_ERROR(rc)) return rc;
	for (i=0;i<NoHandles;i++) {
		void *pciio_tmp = NULL;
		rc = uefi_call_wrapper(BS->OpenProtocol, 6, Handles[i], &PciIoProtocol, &pciio_tmp, ImageHandle, NULL, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
		if (EFI_ERROR(rc)) continue;
		*pciio = pciio_tmp;
...
EFI_STATUS efi_main(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *systab) {
	...
	if (rc == EFI_NOT_FOUND) {
		...
		return rc;
	} else if (EFI_ERROR(rc)) return rc;
	rc = uefi_call_wrapper(pciio->Pci.Read, 5, pciio, EfiPciIoWidthUint32, EFI_FIELD_OFFSET(lpcif_t, rcba), 1, &lpcif.rcba);
	if (EFI_ERROR(rc)) return rc;
	if (!(lpcif.rcba & 1)) {
	...
	set_bit((uint32_t *)(intptr_t)(lpcif.rcba + GCS_OFFSET_ADDR), GCS_RPR_SHIFT, GCS_RPR_PCI);
	return EFI_SUCCESS;
}
