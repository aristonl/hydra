...
static void fill_boxes(UINT32 *PixelBuffer, UINT32 Width, UINT32 Height) {
	UINT32 y, x = 0;
	EFI_GRAPHICS_OUTPUT_BLT_PIXEL Red = {0, 0, 0xff, 0}, Green = {0, 0xff, 0, 0}, *Color;
	for (y = 0; y < Height; y++)
...
static void draw_boxes(EFI_GRAPHICS_OUTPUT_PROTOCOL *gop) {
	...
	for (i = 0; i < imax; i++) {
		...
		fill_boxes(PixelBuffer, info->HorizontalResolution, info->VerticalResolution);
		uefi_call_wrapper(gop->Blt, 10, gop, (EFI_GRAPHICS_OUTPUT_BLT_PIXEL *)PixelBuffer, EfiBltBufferToVideo, 0, 0, 0, 0, info->HorizontalResolution, info->VerticalResolution, 0);
		return;
	}
