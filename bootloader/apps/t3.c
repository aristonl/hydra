...
EFI_STATUS efi_main(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *systab) {
	...
	efi_status = uefi_call_wrapper(BS->HandleProtocol, 3, image_handle, &loaded_image_protocol, &void_li_p);
	li = void_li_p;
	Print(L"%xh (%r)\n", efi_status, efi_status);
	if (efi_status != EFI_SUCCESS) return efi_status;
	Print(L"  li: %xh\n", li);
	if (!li) return EFI_UNSUPPORTED;
