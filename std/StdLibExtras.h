...
#include <std/StdLibExtraDetails.h>
...
//
// clang-format off
#ifndef std_DONT_REPLACE_STD
namespace std {

// NOTE: These are in the "std" namespace since some compilers and static analyzers rely on it.

    template<typename T>
    constexpr T&& forward(std::Detail::RemoveReference<T>& param)
{
    return static_cast<T&&>(param);
}

template<typename T>
constexpr T&& forward(std::Detail::RemoveReference<T>&& param) noexcept
{
